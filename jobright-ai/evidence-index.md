...
Every finding used in the [teardown](./teardown.md), with its evidence type, status, and where it lives. Seven redacted screenshots are **published** in [`img/`](./img/README.md), each with a caption saying what to notice. Raw originals contain personal information and stay out of git by policy; a finding citing one of those is marked "archived (redaction pending)".
...
| JR-08 | 07-08/09 | Tailored resume invented a quantified claim: "improving MVP release timelines by 18%" | Observed (screenshot) | [`resume_make_up_with_quantified_results.png`](./img/resume_make_up_with_quantified_results.png) **published** | High | §5 #1, TL;DR | Verified |
| JR-09 | 07-08/09 | "Name of Latest Employer" autofilled with a consulting firm I never worked at | Observed (screenshot) | [`wrong_info_about_last_employment_info.png`](./img/wrong_info_about_last_employment_info.png) **published** | High | §5 #1, TL;DR | Verified |
...
| JR-13 | 07-08/09 | Compensation expectation filled **6500095000**: saved range 65000 to 95000 concatenated | Observed (screenshot) | [`put_old_salary_range_for_new_postion_crazy_number.png`](./img/put_old_salary_range_for_new_postion_crazy_number.png) **published** | High | §5 #2, TL;DR | Verified |
| JR-14 | 07-08/09 | Form instruction 'If "Yes", type Yes' answered with "relocating" | Observed (screenshot) | [`aigc_is_wrong.png`](./img/aigc_is_wrong.png) **published** | High | §5 #2 | Verified |
...
| JR-16 | 07-08/09 | Status panel "5/5 filled, form complete, Submit Now" while the ATS flagged missing required email and phone | Observed (screenshot) | [`can_not_finish_auto_fill_with_even_simply_fields.png`](./img/can_not_finish_auto_fill_with_even_simply_fields.png) **published** | High | §5 #7 | Verified |
...
| JR-20 | 07-08/09 | "11/11 required fields filled, Submit Now" while the ATS flagged the missing **resume** and current company | Observed (screenshot) | [`submit-now-over-missing-resume.png`](./img/submit-now-over-missing-resume.png) **published** | High | §5 #7, TL;DR | Verified |
...
| JR-23 | 07-08/09 | "Autofill Not Supported" on non-partnered site; manual form left empty | Observed (screenshot) | [`autofill_not_avalable.png`](./img/autofill_not_avalable.png) **published** | High | §5 #3 | Verified |
...
| JR-25 | 07-08/09 | Posting stating "W-2 only, no H-1B sponsorship" served to a profile requiring future sponsorship | Observed (screenshot) | [`autofill_not_avalable.png`](./img/autofill_not_avalable.png) (JD panel, same image) **published** | High | §3①, §4 | Verified |
